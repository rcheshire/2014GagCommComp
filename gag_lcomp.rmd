...
```
**Age Composition**
The commercial handline age compositions were adjusted for bias in sample collections using the Chih method as in SEDAR 10.  
Describe Chih method here....
The commercial diving age compositions were weighted by the state-specific landings in gutted pounds.  For some years the weighting had no effect since all the age data was from a single state.
...
```
**Table 7.**  Commercial handline weighted age compostion (years with 30 or more fish).
```{r commhlacomp,results='asis',echo=FALSE}
table7=xtable(acomp.hl.w.norm3,digits=4)
...
table8=xtable(dv.acomp.norm.w.trim3,digits=4)
